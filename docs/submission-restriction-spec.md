...
Citations are written `<path>@bfab0de:<line>`. This repository configures no
remote for that tree, so a reader adds one to reproduce a citation:

```bash
git -C <engine-checkout> remote add upstream <challenger-repo-url>
git -C <engine-checkout> fetch upstream
git -C <engine-checkout> show bfab0de:<path> | sed -n '<line>p'
```
...
`docs/bonsai2-27b-port-notes.md` section 3.5). The compensating control
...
**A1 (addition) — the measurement harness is untouchable by construction.**
The overlay refuses to act on any editable entry equal to, or inside,
`benchd-bin`, `benchd`, `benchd.pin` or `.gitmodules`. benchd is what MEASURES
a submission, and an editable entry over it would let a submission repoint its
own scorer. `benchd-bin` is where `tools/fetch-benchd.sh` installs the resolved
binary; `benchd` and `benchd.pin` are the retired submodule and pin-file
spellings, and they stay in the refusal list so a returning spelling cannot
reopen the hole. The original had no such surface to protect.
`tools/lint-benchmark-manifest.py` already asserted the manifest side; this
makes the overlay refuse independently of the linter having run.
...
`BENCHD.PIN` names the same file as `benchd.pin`: a byte-comparison guard
passes it and the overlay's `rm -rf "${target_path}"` then deletes the pinned
scorer before writing the submission's copy over it. ASCII folding normalises
the spellings it reaches; a `stat` device and inode comparison over every prefix
of the entry catches the ones it does not. The same fold is applied in
`tools/lint-benchmark-manifest.py` (plus `os.path.samefile`) and in
`enforce-modifiable-surface.sh`, which previously carried no such guard at
all — it was the one layer of the three that would have admitted the write had
the trusted contract ever drifted to list the harness.
...
* **`verify-trusted-source-scope.sh`** — the fifth load-bearing gate named in
  ...
  steer it. Addition A1 here is the same idea narrowed to the measurement
  harness: `benchd-bin`, `.gitmodules`, and the retired `benchd` and
  `benchd.pin` spellings.
