...
#pragma unroll
  ...
  dispatch_bool(!is_unaligned_sm, [&](auto kAlignedM) {
    dispatch_bool(aligned_N || !is_unaligned_bn, [&](auto kAlignedN) {
      ...
      // Four panel stores. Column start stays (tid.x + p) * BN.
      {
        const int col0 = y_col + 0 * BN;
        if (col0 < N) {
          Dpanel[0].store(y + tm * N + tn + 0 * BN, N);
        }
        const int col1 = y_col + 1 * BN;
        if (col1 < N) {
          Dpanel[1].store(y + tm * N + tn + 1 * BN, N);
        }
        const int col2 = y_col + 2 * BN;
        if (col2 < N) {
          Dpanel[2].store(y + tm * N + tn + 2 * BN, N);
        }
        const int col3 = y_col + 3 * BN;
        if (col3 < N) {
          Dpanel[3].store(y + tm * N + tn + 3 * BN, N);
        }
      }
    });
  });
