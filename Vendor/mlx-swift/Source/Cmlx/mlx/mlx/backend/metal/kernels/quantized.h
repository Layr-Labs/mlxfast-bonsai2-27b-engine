...
[[kernel]] void affine_qmv_fast(
  ...
  // Float16 scale loads are valid only when T is float16. The host
  // promotes bfloat16 x with float16 scales to float32 and casts the
  // scale buffer to that dtype before launch. A float32 1.0 is the bytes
  // 00 00 80 3f; reading those bytes as float16 yields 0.
  if constexpr (metal::is_same_v<T, float16_t>) {
    qmv_fast_impl<
        T,
        group_size,
        bits,
