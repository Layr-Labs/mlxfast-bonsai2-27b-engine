...
METAL_FUNC void qmm_t_nax_tgp_impl(
  ...
  // A simdgroup whose whole row slab lies at or beyond M owns no output. It
  // still takes part in the cooperative weight loads and the barriers below,
  // but it skips its A-tile loads, its tensor products and its store.
  const bool sgp_has_rows = sgp_sm > 0;
