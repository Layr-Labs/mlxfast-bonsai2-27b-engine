...
extension EngineLoopV2 {
    ...
    /// Default ON, as on the record d5c6433 (`BONSAI_EARLY_REPLAY=0` leaves
    /// the committed recurrent state lazy). The off default came from trees
    /// that committed the verify in slices, where the lazy replay rides in
    /// the verify's first slice; this tree runs the verify as one submission
    /// (slices off), and every ranked tree that showed the fast verify round
    /// (polymorf 5fbfa003, ercumentyildirim 2530f2a) submitted it early.
