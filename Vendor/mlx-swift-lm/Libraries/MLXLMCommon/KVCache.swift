...
/// `RotatingKVCache` for the DFlash 2 block drafter's context rows, written
/// in place while the window has room.
///
/// The drafter appends `c` context rows (the previous round's confirmed
/// columns, `1...16`) per round, and `RotatingKVCache.update` routes every
/// multi-row append through `updateConcat`, which re-copies the whole
/// context (`[1, 8, ctx, 128]` K and V) per layer per round. The drafter
/// then concatenates the block's own 16 keys and values after the cache
/// once more. Here one buffer of `maxSize + block` rows per K and V is
/// allocated once, and `updateBlock` writes the round's context AND block
/// rows at the current end by slice assignment (`mlx_slice_update`, in
/// place when the buffer is not shared, exactly as `CBv2FullSequenceKV`
/// appends), so a round costs the `c + block` row write, not a copy of the
/// context. The attention reads `[..., ..<rows + c + block, :]` views. The
/// block rows sit past the cache's end and are overwritten by the next
/// round's context rows; `offset` advances by the context rows only, as
/// before.
///
/// While `rows + c <= maxSize` the parent's path would have appended
/// without trimming, so the held rows are the same rows in the same order.
/// The first append that would exceed the window (or any plain `update`)
/// hands the held rows to the parent's rotating path, which then owns the
/// state for good, so the sliding-window semantics past the window are the
/// parent's. `DARKBLOOM_DFLASH2_KV_INPLACE=0` makes the drafter build the
/// plain `RotatingKVCache` instead.
public final class DFlash2BlockKVCache: RotatingKVCache {
    public static let enabled: Bool = {
        let value = ProcessInfo.processInfo.environment["DARKBLOOM_DFLASH2_KV_INPLACE"]?
    ...
    private var rows = 0
    /// True once the parent's rotating path owns the state.
    private var rotating = false

    /// Write `keys`/`values` (`[B, H, contextRows + blockRows, D]`, the
    /// context rows first) at the buffer's end, advance the offset by
    /// `contextRows`, and return views over every held row plus the block.
    /// Returns nil when the window would overflow or the parent already
    /// owns the state; the caller then takes the plain `update` path.
    public func updateBlock(
        keys: MLXArray, values: MLXArray, contextRows: Int
    ) -> (MLXArray, MLXArray)? {
        ...
        if self.keys == nil || self.keys!.dim(2) < end {
            ...
            let vShape = [values.dim(0), values.dim(1), capacity, values.dim(3)]
            let newK = MLXArray.zeros(kShape, dtype: keys.dtype)
            let newV = MLXArray.zeros(vShape, dtype: values.dtype)
            if let k = self.keys, let v = self.values, rows > 0 {
                self.keys = concatenated([k[.ellipsis, ..<rows, 0...], newK], axis: 2)
                self.values = concatenated([v[.ellipsis, ..<rows, 0...], newV], axis: 2)
            } else {
                self.keys = newK
                self.values = newV
            }
    ...
    private func handOverToRotation() {
        ...
        if let k = self.keys, let v = self.values {
            self.keys = k[.ellipsis, ..<rows, 0...]
            self.values = v[.ellipsis, ..<rows, 0...]
        }
        idx = rows
        rotating = true
    }
    ...
    public override var state: [MLXArray] {
        get {
            guard !rotating, let keys = self.keys, let values = self.values else {
                return super.state
            }
            return [keys[.ellipsis, ..<rows, 0...], values[.ellipsis, ..<rows, 0...]]
        }
        set {
            // Restored state is the parent's to own: its `metaState` carries
            // the rotation cursor, which the in-place path does not track.
            rotating = true
    ...
    public override func trim(_ n: Int) -> Int {
        ...
        if !rotating { rows = Swift.max(0, rows - trimmed) }
        return trimmed
    }
}
