...
public final class DFlash2DraftModel: Module, @unchecked Sendable {
    ...
    func logits(_ hidden: MLXArray) throws -> MLXArray {
        guard let target else { throw DFlash2Error.notBound }
        // The head is the target's packed Hadamard module with FP16 scales.
        // A BF16 input promotes to FP32 inside `quantized_matmul` anyway
        // (bf16 x f16 -> f32), and on that path the module's FP32 constant
        // cache does not apply (`cachedCast` serves FP32 inputs only), so the
        // full-vocabulary scales and offsets were widened afresh every round.
        // An FP32 input takes the same cached path the target's own head
        // takes, and keeps the rotated input at FP32 instead of BF16.
        var logits = target.logitsForDFlash2Hidden(hidden.asType(.float32))
