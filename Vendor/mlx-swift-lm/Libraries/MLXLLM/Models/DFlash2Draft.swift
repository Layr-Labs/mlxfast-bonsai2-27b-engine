...
private final class DFlash2Attention: Module {
    ...
    private let preworkGeometry: DFlash2AttentionPrework.Prepared?
        ...
        if dflash2KVConcatEnabled {
            ...
            if let geometry = preworkGeometry,
                ...
                let fused = DFlash2AttentionPrework.run(
                    ...
                    prepared: geometry, base: preworkBase)
            {
...
private enum DFlash2AttentionPrework {
    ...
    /// Shapes are part of the MLX compile cache. Offsets and current norm
    /// weights stay dynamic inputs, so no request value or parameter is frozen.
    /// Scalar geometry arrays and Metal launch metadata are built when a shape
    /// is traced rather than on every round. Kept outside the module tree.
    final class Prepared: Sendable {
        ...
        init(_ geometry: Geometry) {
            ...
            self.call = compile(shapeless: false) { (inputs: [MLXArray]) -> [MLXArray] in
                let q = inputs[0]
                let k = inputs[1]
                return DFlash2AttentionPrework.kernel(
                    [q, k, inputs[2], inputs[3], MLXArray(geometry.eps), MLXArray(geometry.eps),
                     inputs[4], inputs[5], MLXArray(log2(geometry.base))],
    ...
    nonisolated(unsafe) private static var prepared: [Geometry: Prepared] = [:]
    ...
    static func run(
        ...
        qOffset: Int, kOffset: Int, prepared: Prepared, base: Float
    ) -> (MLXArray, MLXArray)? {
        ...
        return launch(q: q, k: k, wq: qNorm.weight, wk: kNorm.weight,
            qOffset: qOffset, kOffset: kOffset, prepared: prepared)
    }

    private static func launch(
        q: MLXArray, k: MLXArray, wq: MLXArray, wk: MLXArray,
        qOffset: Int, kOffset: Int, prepared: Prepared
    ) -> (MLXArray, MLXArray) {
        let result = prepared.call(
            [q, k, wq, wk, MLXArray(Int32(qOffset)), MLXArray(Int32(kOffset))])
        return (result[0], result[1])
    }

    static func prepare(heads: Int, kvHeads: Int, base: Float, eps: Float) -> Prepared? {
        ...
        return lock.withLock {
            if let passed = verdicts[geometry] { return passed ? prepared[geometry] : nil }
            let candidate = Prepared(geometry)
            let passed = selfCheck(candidate)
            ...
            if passed { prepared[geometry] = candidate }
            return passed ? candidate : nil
        }
    }

    private static func selfCheck(_ prepared: Prepared) -> Bool {
        let geometry = prepared.geometry
        do {
            return try withError { error in
                    ...
                    .enumerated()
                {
                    ...
                    let fused = launch(q: q, k: k, wq: wq, wk: wk,
                        qOffset: qOffset, kOffset: offset, prepared: prepared)
