...
/// The speculative leg's prompt forward. Without this conformance the engine's
/// MTP prefill (`forwardWithHiddenForPrefill`) falls back to
/// `cbv2ForwardWithHidden` and projects EVERY prompt row through the
/// vocabulary head, then keeps only the last row (`narrowPrefillOutput`). This
/// keeps the whole-row hidden -- the committed-history contract the assistant
/// reads -- and projects only what the requirement names, with the SAME ops
/// `cbv2RecurrentPrefill` gives the serial engine's prompt chunk: final
/// RMSNorm is row-independent, so norm-after-slice equals slice-after-norm for
/// the surviving row, and the trunk (every K/V write, every recurrent stage,
/// the block drafter's layer tap) is the identical `model.cbv2Forward` call.
