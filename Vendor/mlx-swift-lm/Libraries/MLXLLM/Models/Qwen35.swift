...
final class Qwen35MRoPE {
    ...
    /// One-entry shared memo for the default-path (cosine, sine) tables.
    ///
    /// All 16 attention layers apply the same positions at the same rotaryDim
    /// in one forward, so layers 2..16 reuse layer 1's tables instead of
    /// rebuilding cos/sin per layer per round. Keyed by the incoming positions
    /// array's identity (same object means same values; MLX arrays are
    /// immutable) plus rotaryDim, and storing the tables pre-cast, so a hit is
    /// bit-identical to recomputing: the consumer casts exactly as before. A
    /// miss recomputes with the same ops and replaces the entry. This is a
    /// RoPE table keyed on shapes and offsets, shared within one forward; it
    /// never carries values across runs. Guarded for concurrent streams; at
    /// worst a race recomputes.
    // All accesses hold `defaultTableCacheLock`.
    nonisolated(unsafe) private static var defaultTableCache: (
        key: MLXArray, rotaryDim: Int, cosine: MLXArray, sine: MLXArray
    )?
    private static let defaultTableCacheLock = NSLock()
    ...
    /// Default-path (cosine, sine) tables for one positions array, shared
    /// across the attention layers of a forward (see `defaultTableCache`).
    /// `key` is the incoming positions object, whose identity is the cache
    /// key; `positions` is the same array normalized to 3 planes. Tables are
    /// memoized pre-cast; the consumer's cast and expand run per call exactly
    /// as they did before the memo, so hits are bit-identical to misses.
    private func sharedDefaultTables(
        key: MLXArray, positions: MLXArray, invFreq: MLXArray, dtype: DType
    ) -> (MLXArray, MLXArray) {
        Self.defaultTableCacheLock.lock()
        let cached = Self.defaultTableCache
        Self.defaultTableCacheLock.unlock()
        if let cached, cached.key === key, cached.rotaryDim == rotaryDim {
            return (
                cached.cosine.asType(dtype).expandedDimensions(axis: 1),
                cached.sine.asType(dtype).expandedDimensions(axis: 1))
        }
        ...
        let sine = sin(angles)
        Self.defaultTableCacheLock.lock()
        Self.defaultTableCache = (
            key: key, rotaryDim: rotaryDim, cosine: cosine, sine: sine)
        Self.defaultTableCacheLock.unlock()
        return (
            cosine.asType(dtype).expandedDimensions(axis: 1),
            sine.asType(dtype).expandedDimensions(axis: 1))
    }
    ...
    func apply(
        ...
        if let defaultInvFreq {
            let (cosine, sine) = sharedDefaultTables(
                key: positionIds, positions: positions,
                invFreq: defaultInvFreq, dtype: queries.dtype)
