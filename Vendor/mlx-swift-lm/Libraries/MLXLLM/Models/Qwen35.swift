...
/// forward. Windows under `minRows` (verify, replay, decode) never take this
/// path. A window that is not a whole number of chunks runs its remainder rows
/// on the sequential kernel from the chunked state. `MLXFAST_GDN_CHUNKED=0`
/// turns the path off; `MLXFAST_GDN_CHUNK` picks the chunk (8, the default,
/// or 16).
enum Qwen35GatedDeltaChunked {
    ...
    private static let prepSource = """
            ...
            for (int h0 = 0; h0 < HR; h0 += GP) {
              ...
              // in-chunk inclusive prefix of log g within each lane group
              float gam = 0.0f;
              float bet = 0.0f;
              if (live) {
                gam = metal::precise::log(g[((size_t)b_idx * T_ + t0 + row) * Hv + hv]);
              ...
              if (live) {
                ...
                gf_[row] = metal::precise::exp(gam);
                gf_[C + row] = metal::precise::exp(gam_last - gam);
              }
              ...
              for (int j = 0; j < C; ++j) {
                ...
                if (live) {
                  ...
                  float av = 0.0f;
                  if (j <= row) {
                    const float e = metal::precise::exp(gam - gj);
                    pv = QK[row * LD + j] * e;
