...
final class Qwen35Attention: Module {
    ...
    func cbv2Forward(
        ...
        quantizedInput: SignedBlockHadamard.Int8Activation? = nil,
        ropeTables: (cosine: MLXArray, sine: MLXArray)? = nil
    ) -> MLXArray {
        let B = x.dim(0)
        let L = x.dim(1)
                ...
                offsets: cache.positionOffsets)
        {
            ...
            // A forward-wide shared table pair (built once by the model) skips
            // the per-layer rebuild; otherwise each layer builds its own.
            if let positionIds {
                // Shared tables must cover this layer's batch, length and
                // rotary width; anything else falls back to the per-layer build.
                if let ropeTables, ropeTables.cosine.dim(0) == B,
                    ropeTables.cosine.dim(2) == L,
                    ropeTables.cosine.dim(-1) == mrope.rotaryDim
                {
                    (queries, keys) = mrope.applyWithTables(
                        queries: queries, keys: keys, tables: ropeTables)
                } else {
...
final class Qwen35MRoPE {
    ...
    /// Default-path (cosine, sine) tables for normalized 3-plane `positions`,
    /// built once per forward and shared across the attention layers (see the
    /// model's `ropeTables`). FP32 and post-expanded; each consumer casts to
    /// its queries' dtype exactly as the inline path did, so shared tables are
    /// bit-identical to per-layer ones. Nil when the non-default
    /// (per-frequency) path applies.
    func buildTables32(_ positions: MLXArray) -> (cosine: MLXArray, sine: MLXArray)? {
        guard positions.ndim == 3, positions.dim(0) == 3,
            rotaryDim % 2 == 0, let defaultInvFreq
        else { return nil }
        ...
        return (
            cos(angles).expandedDimensions(axis: 1),
            sin(angles).expandedDimensions(axis: 1))
    }

    /// Default-path tables for raw `positionIds` (normalized here), for the
    /// model to build once per forward. See `buildTables32`.
    func sharedTables(positionIds: MLXArray) -> (cosine: MLXArray, sine: MLXArray)? {
        ...
        return buildTables32(positions)
    }

    /// `apply` over caller-provided shared tables (same values `apply` would
    /// build for these positions). The cast runs per call exactly as before.
    func applyWithTables(
        queries: MLXArray, keys: MLXArray,
        tables: (cosine: MLXArray, sine: MLXArray)
    ) -> (MLXArray, MLXArray) {
        let cosine = tables.cosine.asType(queries.dtype)
        let sine = tables.sine.asType(queries.dtype)
    ...
    func apply(
        ...
        if let tables = buildTables32(positions) {
            return applyWithTables(queries: queries, keys: keys, tables: tables)
        }
    ...
    func cbv2Forward(
        ...
        if isLinear {
            ...
            r = selfAttn!.cbv2Forward(
                ...
                exactTargetVerify: exactTargetVerify, ropeTables: ropeTables)
        }
        let h = x + r
        let feedForward: MLXArray
        if let sparse = mlp as? Qwen35SparseMoeBlock {
...
public class Qwen35TextModelInner: Module {
    ...
    func cbv2Forward(
        ...
        // One shared M-RoPE table pair per forward for explicit positions: the
        // 16 attention layers apply the same positions at the same rotaryDim,
        // so the first layer's tables serve all of them (each consumer casts
        // to its queries' dtype exactly as before: bit-identical). Nil when
        // positions are absent or the non-default path applies; layers then
        // build their own tables exactly as before.
        let ropeTables: (cosine: MLXArray, sine: MLXArray)? = positionIds.flatMap { positions in
            layers.lazy.compactMap { $0.selfAttn?.mrope }.first?.sharedTables(
