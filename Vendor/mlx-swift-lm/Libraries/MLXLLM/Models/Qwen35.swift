...
public class Qwen35TextModelInner: Module {
    ...
    /// Prompt slices: default OFF here (`BONSAI_PROMPT_SLICES=1` turns them
    /// on). Every tree carrying them on the ranked box read its decode window
    /// in the slow state in all three pairs (`9024f66b`, `6c565550`,
    /// `3baf5cb4`, `fd020d92`, `bf5201cc`: 4.93-5.04 ms/token), while the
    /// record lineage without them (`2530f2a0`, `ccb885d6`) read ~4.63-4.70.
    static let promptSlices: Bool = {
        let value = ProcessInfo.processInfo.environment["BONSAI_PROMPT_SLICES"]?
            .trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        return ["1", "true", "yes", "on"].contains(value ?? "")
    }()
