...
enum Qwen35TrunkSubmission {
    ...
    /// Off by default: prompt-phase early submission changes the buffer
    /// allocation pattern the decode phase inherits, and every tree that
    /// carries it has measured the slow verify mode (control pair:
    /// `ccb885d6` window 4.636 -> `6751fef2` 4.969, one 12-line change).
    /// `MLXFAST_PREFILL_PIPELINE` restores a plan.
