...
final class Qwen35GatedDeltaNet: Module {
    ...
    /// `in_proj_b` and `in_proj_a` with the input signs folded in.
    private let signedB = Qwen35SignFoldedWeight()
    private let signedA = Qwen35SignFoldedWeight()
    ...
    private func projectInputs(
            ...
            let shared = sharedHadamardProjectionsPreSigned(inputs, [inProjQKV, inProjZ])
        {
            ...
            let wb = signedB.weight(inProjB.weight, signs: signs)
            let wa = signedA.weight(inProjA.weight, signs: signs)
