...
enum Qwen35TrunkSubmission {
    ...
    static let verify: Plan = {
        ...
        // Submit after the first 4 layers once, then after 8, while the host
        // builds the rest. An earlier first submit starts the GPU sooner on
        // small leading graphs; keeping 8 preserves the promoted boundary.
        // Keep explicit plans and both existing disable switches available.
        return Plan.parse(
            env["MLXFAST_VERIFY_SLICE_LAYERS"],
            default: Plan(stride: 0, offset: 0, explicit: [4, 8]))
    }()
