...
enum Qwen35GatedDeltaChunked {
    ...
    /// Off by default: `MLXFAST_GDN_CHUNKED_VERIFY=1` takes the chunked kernels
    /// at verify width. On the ranked box the capture-verify window is where
    /// the chunked scan is the *worse* kernel: it pays the prep pass, the
    /// cross-chunk state propagation and the level-1 output fixup for a window
    /// that is already only two chunks wide (`T = 16`, `chunk = 8`), while the
    /// register-resident sequential kernel does the same 16 rows with no prep
    /// and no extra state traffic. The ranked phase numbers say the same thing
    /// from the outside: the tree that carries the chunked verify (`a255ab16`,
    /// 4.914 ms per window token) is 5.9% slower in the decode window than its
    /// parent `5fbfa003` (4.632), which does not carry it, while their
    /// prompt-width chunked scan is what buys the prompt phase. The prompt
    /// width never consults this flag (`run`/`window` above, gated by
    /// `MLXFAST_GDN_CHUNKED`), so the prompt and the seed prefill are unchanged
    /// by this default.
