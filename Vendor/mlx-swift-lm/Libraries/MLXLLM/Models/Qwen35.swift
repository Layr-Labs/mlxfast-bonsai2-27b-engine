...
final class Qwen35GatedDeltaNet: Module {
    ...
    /// the rotation reads the same values either way. With FP32 operands the
    /// tail `silu(gate) * normed`, the signs and the rotation run as one pass
    /// (`HadamardQuantizedLinear.forwardAfterSwiGLU`, the same FP32 products
    /// in the same order as the tail then `rotate`).
    private func projectGatedOut(_ out: MLXArray, gate: MLXArray, B: Int, S: Int) -> MLXArray {
        if let packed = outProj as? HadamardQuantizedLinear, out.dtype == .float32,
            gate.dtype == .float32, norm.weight.dtype == .float32, gate.shape == out.shape
        {
            let normed = MLXFast.rmsNorm(out, weight: norm.weight, eps: norm.eps)
            if let y = packed.forwardAfterSwiGLU(
                gate: gate.reshaped(B, S, -1), up: normed.reshaped(B, S, -1),
                widenOutput: false)
            {
                return y
            }
    ...
    func cbv2ForwardCaptured(
        ...
        let out: MLXArray
        if S >= 3 {
            // Gates once for the window (the prework kernel already forms
            // them); the accepted-prefix replay slices them instead of
            // recomputing them from the taped a and b.
            let windowGates: [MLXArray] =
                pre.map { [$0.g, $0.beta] }
                ?? Qwen35FusedElementwise.gatedDeltaGates([a, b, aLog, dtBias])
...
final class Qwen35Attention: Module {
    ...
    /// q, k and v when they are packed with one shared input transform and no
    /// GDN layout permutation precedes its signs; the decoder layer then folds
    /// those signs into the input norm.
    var sharedInputSiblings: [HadamardQuantizedLinear]? {
        guard let siblings = sharedHadamardSiblings([qProj, kProj, vProj]),
            siblings.allSatisfy({ $0.gdnLayout == nil })
        else { return nil }
        return siblings
    }
    ...
    func cbv2Forward(
        ...
        preSignedSiblings: [HadamardQuantizedLinear]? = nil
    ) -> MLXArray {
        let B = x.dim(0)
        let L = x.dim(1)
        ...
        if let preSignedSiblings {
            // `x` already carries the shared transform's signs (folded into
            // the input norm's gain): the same rotation and the same fused
            // sibling product `projectQKV` forms, minus the sign multiply.
            precondition(!exactTargetVerify, "Qwen35 exact verify takes the unsigned input")
            guard let shared = sharedHadamardProjectionsPreSigned(x, preSignedSiblings) else {
...
final class Qwen35DecoderLayer: Module {
    ...
    private let inputSignedGain = Qwen35SignedGain()
    ...
    func cbv2Forward(
        ...
        if isLinear {
            ...
            if !exactTargetVerify, Qwen35FusedElementwise.foldsHadamardSigns,
                ...
                inputLayerNorm.weight.ndim == 1, inputLayerNorm.weight.dtype == .float32,
                x.dtype == .float32,
                let siblings = attention.sharedInputSiblings,
                let transform = siblings.first?.transform,
                inputLayerNorm.weight.dim(0) == transform.width
            {
                // q/k/v's signs ride in the input norm's gain, as gate/up's
                // ride in the post-attention gain (see `Qwen35SignedGain`).
                let signedInput = MLXFast.rmsNorm(
                    x,
                    weight: inputSignedGain.gain(
                        inputLayerNorm.weight, signs: transform.signVector),
                    eps: inputLayerNorm.eps)
                r = attention.cbv2Forward(
                    signedInput, cache: attentionCache, positionIds: positionIds,
                    preSignedSiblings: siblings)
            } else {
                r = attention.cbv2Forward(
