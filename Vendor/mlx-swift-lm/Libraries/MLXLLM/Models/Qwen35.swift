...
public class Qwen35TextModelInner: Module {
    ...
    func cbv2Forward(
        ...
        for (modelLayerIndex, layer) in layers.enumerated() {
            ...
            // Verify slices: a speculative verify window is built on the host
            // while the drafter runs on the GPU. Submitting the tower in
            // slices lets the GPU start the first layers as soon as they are
            // built instead of after the whole window's graph exists. The
            // graph is unchanged; only its submission is split.
            if captureRecurrentWindow, Qwen35VerifySlices.enabled,
                Qwen35VerifySlices.boundaries.contains(modelLayerIndex)
            {
...
/// Verify-slice submission points (see `Qwen35TextModelInner.cbv2Forward`).
/// `BONSAI_VERIFY_SLICES=0` submits the verify window as one graph again.
enum Qwen35VerifySlices {
    static let enabled: Bool = {
        let value = ProcessInfo.processInfo.environment["BONSAI_VERIFY_SLICES"]?
            .trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        return !["0", "false", "no", "off"].contains(value ?? "")
    }()
    /// Output layers after which the running hidden state is submitted.
    static let boundaries: Set<Int> = [7, 15, 31, 47]
}
