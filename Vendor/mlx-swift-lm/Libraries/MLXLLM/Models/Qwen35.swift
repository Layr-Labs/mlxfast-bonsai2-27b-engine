...
public class Qwen35TextModelInner: Module {
    ...
    /// On unless `DARKBLOOM_QWEN35_VERIFY_SLICES=0`. See `cbv2Forward`.
    static let verifySliceSubmission: Bool = {
        let value = ProcessInfo.processInfo.environment["DARKBLOOM_QWEN35_VERIFY_SLICES"]?
            .trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        return !["0", "false", "no", "off"].contains(value ?? "")
    }()
    /// Sixteen layers per slice: four slices of the 64-layer tower.
    static let verifySliceLayers = 16
    ...
    func cbv2Forward(
        ...
        for (modelLayerIndex, layer) in layers.enumerated() {
            ...
            // Verify and prompt forwards: hand the tower to the GPU in slices,
            // so the front runs while the host builds the back instead of the
            // GPU idling until the whole graph is built. Same kernels, same
            // inputs, same order; only command-buffer boundaries move.
            // Meganpark980320's change (queued a7303ad5 / 7f634f66, measured
            // -3.0% decode window); the prompt side follows i34-9's queued
            // 4bf0c89c ("prefill pipeline").
            if captureRecurrentWindow || inputs.dim(1) > 32, Self.verifySliceSubmission,
                (modelLayerIndex + 1) % Self.verifySliceLayers == 0,
