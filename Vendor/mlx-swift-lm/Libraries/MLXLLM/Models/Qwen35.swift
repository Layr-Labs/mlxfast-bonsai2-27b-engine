...
enum Qwen35TensorPackedMatmul {
    ...
    enum PackedOperandSupport { case native2b, staged4b, none }
    ...
    /// ending the process).
    private static func probe(_ name: String, _ source: String) -> Bool {
        probeFailed = false
        ...
        withErrorHandler({ _ in Qwen35TensorPackedMatmul.probeFailed = true }) {
            let a = MLXArray.zeros([16, 128], dtype: .float16)
    ...
    static func installIfNeeded() {
        ...
        HadamardQuantizedLinear.tensorPackedMatmul = {
            ...
            let packedKernel = support == .native2b ? kernel : kernelStaged
            return packedKernel(
