...
enum Qwen35TensorPackedMatmul {
    ...
    // 128], ksz int32 [K, M, N]. Template: OutT, MPERM, VEC.
    ...
    /// The native (`uint2b_format`) prompt kernel's final write (`source`) as
    /// one 4-wide store per group of four accumulators, the same grouping and
    /// the same `OutT` values at the same addresses as `staged8VectorStores`
    /// (dukemawex `5a1a96ec`, carried byte for byte in ercumentyildirim
    /// `6e19fe1`). `nb` is 4-aligned and `N % 64 == 0` (the route's guard).
    /// `MLXFAST_RIDER_Q8_VEC=0` keeps the scalar stores.
    static let nativeVectorStores: Bool = {
        let value = ProcessInfo.processInfo.environment["MLXFAST_RIDER_Q8_VEC"]?
            .trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        return !["0", "false", "no", "off"].contains(value ?? "")
    }()
    ...
    /// The A-resident verify int8 bodies (L1, `sourceNarrowInt8AR`: A staged
    /// in threadgroup memory or held in a cooperative tensor, B staged
    /// `[K][N]`) as in-situ trial candidates, each after the bitwise self-test
    /// (a body that fails to compile or to match is dropped on its own).
    /// `DARKBLOOM_BONSAI_TENSOR_ROUTE_ARES=0` removes them from the self-test,
    /// the timing and the trial.
    static let aresEnabled: Bool = {
        let value = ProcessInfo.processInfo.environment["DARKBLOOM_BONSAI_TENSOR_ROUTE_ARES"]?
            .trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        return !["0", "false", "no", "off"].contains(value ?? "")
    }()
    ...
    // The verify int8 kernel with the activation operand made resident (L1,
    // `aresEnabled`). Every other body stages B (the weight codes) in
    // threadgroup memory but has the tensor op read A (the 16 activation
    // rows, a 16 x 128 int8 slice at a row stride of K bytes) from device
    // memory on every op. Same threadgroup, same K split, same register
    // rings, same ops on the same integers and the same FP32 epilogue and
    // reduction as `sourceNarrowInt8Pipelined`, so the output is bitwise that
    // kernel's (self-tested at load); what changes is where the op finds its
    // operands:
    // - AM = 1: each K step's 16 x KH slice of A is staged into threadgroup
    //   memory beside the step's B tile (16 rows of KH bytes, 4 uint4 per lane
    //   and group loaded with the words, one ring slot ahead, and stored at the
    //   same simdgroup barriers as B), and the op reads it there.
    // - BL = 1: B is staged `[K][N]` (32 bytes per K row) and read with the
    //   descriptor's right transpose off, instead of `[N][K]` (128-byte column
    //   stride) read transposed. Lane l stages columns 4 (l % 8) .. + 3 of words
    //   l / 8 and 4 + l / 8 of each K half: the same 2-bit planes as the base
    //   staging, the 4 x 4 bytes of each plane transposed in registers, one
    //   32-bit store per K row (rotated by l / 8 so the 32 lanes of a store hit
    //   32 distinct banks). The packed weights are read as stored.
    // Templates: OutT, NEG, F32S, FACTORED (as `sourceNarrowInt8`), PD, TN, KH
    // (as `sourceNarrowInt8Pipelined`), AM, BL.
    // grid (N / TN * 128, 1, 1), threadgroup (128, 1, 1).
    // Bounds: K % 512 == 0 (the route's guard: K / 512 whole groups per
    // simdgroup, no K tail) and N % TN == 0 (`narrowKernel`, the fused head's
    // guard), so every access stays in x [16, K], w [N, K / 16], scalesT
    // [K / 128, N], ascale / rowsum [16, K / 128], out [16, N]; every A slice
    // (AM = 1 loads, AM = 2 cooperative loads) is a whole 16 x KH tile at a
    // 64-byte offset, row stride K. At most 16 KB of threadgroup memory.
    private static let sourceNarrowInt8AR = """
        const int K = ksz[0]; const int M = 16; const int N = ksz[2];
        const int Kg = K / 128;
        const int n0 = int(threadgroup_position_in_grid.x) * TN;
        const uint lane = thread_index_in_simdgroup;
        ...
        const int g1 = g0 + gper;
        constexpr int NH = TN / 32;
        constexpr int NQ = KH / 64;           // K halves (word quads) per staged step
        constexpr int CD = PD < 2 ? 2 : PD;   // constants ring depth = group-loop unroll
        constexpr auto desc = mpp::tensor_ops::matmul2d_descriptor(16, 32, KH, false, BL == 0, false, KH == 64 ? mpp::tensor_ops::matmul2d_descriptor::mode::multiply_accumulate : mpp::tensor_ops::matmul2d_descriptor::mode::multiply);
        mpp::tensor_ops::matmul2d<desc, metal::execution_simdgroup> op;
        tensor<device int8_t, dextents<int, 2>, tensor_inline> A((device int8_t*)x, dextents<int, 2>(K, M));  // SIGNED codes only
        threadgroup uint32_t bs[4][NH][32 * KH / 4];
        threadgroup uint4 ares[4][AM == 1 ? KH : 1];   // AM = 1: the step's 16 x KH slice of A
        const dextents<int, 2> bext = BL == 0 ? dextents<int, 2>(KH, 32) : dextents<int, 2>(32, KH);
        tensor<threadgroup int8_t, dextents<int, 2>, tensor_inline> B0((threadgroup int8_t*)bs[sg][0], bext);
        tensor<threadgroup int8_t, dextents<int, 2>, tensor_inline> B1((threadgroup int8_t*)bs[sg][NH - 1], bext);
        tensor<threadgroup int8_t, dextents<int, 2>, tensor_inline> At((threadgroup int8_t*)ares[sg], dextents<int, 2>(KH, 16));
        auto tA0 = A.template slice<KH, 16>(0, 0);
        auto cT0 = op.template get_destination_cooperative_tensor<metal::remove_addrspace_t<decltype(tA0)>, metal::remove_addrspace_t<decltype(B0)>, int32_t>();
        auto cT1 = op.template get_destination_cooperative_tensor<metal::remove_addrspace_t<decltype(tA0)>, metal::remove_addrspace_t<decltype(B0)>, int32_t>();
        // AM = 2: A as the op's left-input cooperative tensor, one per K step
        // of a group (cA0; KH = 64: cA1 for the upper half), refilled with the
        // next group's slice right after the op that consumed it, so the loads
        // are in flight under the epilogue and the staging. Needs the
        // toolchain's int8 cooperative-input load; where it is missing AM = 2
        // fails to compile (and is dropped at load), AM = 0 / 1 are unaffected.
        ...
        #else
        static_assert(AM != 2, "no int8 cooperative left input on this toolchain");
        #endif
        constexpr int CAP = 32 / 2;
        const int fm = int(((lane >> 4) & 1) * 4 + ((lane >> 1) & 3));
        const int fn = int((((lane >> 3) & 1) * 2 + (lane & 1)) * 4);
        ...
        // BL = 0: lane -> column lane of each 32-column half, v[h][j] = word j of the group.
        // BL = 1: lane -> columns 4 cq + c, v[h][4 q + c] = word 4 q + jl of column 4 cq + c.
        const int cq = int(lane) & 7;
        const int jl = int(lane) >> 3;
        const device uint32_t* wrow = BL == 0
            ? w + (size_t)(n0 + int(lane)) * (K / 16)
            : w + (size_t)(n0 + 4 * cq) * (K / 16) + jl;
        const size_t hstride = (size_t)32 * (K / 16);
        // K halves [q0, q1) of group gg's words into v
        auto getw = [&](int gg, thread uint32_t (&v)[NH][8], int q0, int q1) {
          #pragma clang loop unroll(full)
          for (int h = 0; h < NH; h++) {
            #pragma clang loop unroll(full)
            for (int q = 0; q < 2; q++) {
              if (q < q0 || q >= q1) { continue; }
              if constexpr (BL == 0) {
                const uint4 u = ((const device uint4*)(wrow + h * hstride + (size_t)gg * 8))[q];
                v[h][4 * q + 0] = u.x; v[h][4 * q + 1] = u.y; v[h][4 * q + 2] = u.z; v[h][4 * q + 3] = u.w;
              } else {
                #pragma clang loop unroll(full)
                for (int c = 0; c < 4; c++) {
                  v[h][4 * q + c] = wrow[h * hstride + (size_t)c * (K / 16) + (size_t)gg * 8 + 4 * q];
                }
              }
            }
          }
        };
        // stages K halves [q0, q1) of v into the step's B tile(s)
        auto putw = [&](thread const uint32_t (&v)[NH][8], int q0, int q1) {
          #pragma clang loop unroll(full)
          for (int h = 0; h < NH; h++) {
            if constexpr (BL == 0) {
              ...
              for (int j = 0; j < 8; j++) {
                ...
                // As in the prompt kernel's staging: one uint4 store per word.
                const uint32_t wv = v[h][j];
                *(threadgroup uint4*)(dst + 4 * (j % (KH / 16))) = uint4(
                    wv & 0x03030303u, (wv >> 2) & 0x03030303u,
                    (wv >> 4) & 0x03030303u, (wv >> 6) & 0x03030303u);
              }
            } else {
              threadgroup uint32_t* dst = bs[sg][h];
              #pragma clang loop unroll(full)
              for (int q = 0; q < 2; q++) {
                if (q < q0 || q >= q1) { continue; }
                // K row of the word's first code in the step's tile
                const int rb = (KH == 128 ? 64 * q : 0) + 16 * jl;
                #pragma clang loop unroll(full)
                for (int p = 0; p < 4; p++) {
                  // plane p as the base staging has it: byte b = code 4 b + p,
                  // staged at K row rb + 4 p + b; then bytes (column c, row b)
                  // transposed so each K row's word holds columns 4 cq .. + 3.
                  const uint32_t y0 = (v[h][4 * q + 0] >> (2 * p)) & 0x03030303u;
                  const uint32_t y1 = (v[h][4 * q + 1] >> (2 * p)) & 0x03030303u;
                  const uint32_t y2 = (v[h][4 * q + 2] >> (2 * p)) & 0x03030303u;
                  const uint32_t y3 = (v[h][4 * q + 3] >> (2 * p)) & 0x03030303u;
                  const uint32_t t0 = (y0 & 0x00FF00FFu) | ((y1 & 0x00FF00FFu) << 8);
                  const uint32_t t1 = ((y0 >> 8) & 0x00FF00FFu) | (y1 & 0xFF00FF00u);
                  const uint32_t t2 = (y2 & 0x00FF00FFu) | ((y3 & 0x00FF00FFu) << 8);
                  const uint32_t t3 = ((y2 >> 8) & 0x00FF00FFu) | (y3 & 0xFF00FF00u);
                  const uint32_t o0 = (t0 & 0xFFFFu) | (t2 << 16);
                  const uint32_t o1 = (t1 & 0xFFFFu) | (t3 << 16);
                  const uint32_t o2 = (t0 >> 16) | (t2 & 0xFFFF0000u);
                  const uint32_t o3 = (t1 >> 16) | (t3 & 0xFFFF0000u);
                  #pragma clang loop unroll(full)
                  for (int t = 0; t < 4; t++) {
                    const int b = (t + jl) & 3;
                    const uint32_t o = (b & 2) ? ((b & 1) ? o3 : o2) : ((b & 1) ? o1 : o0);
                    dst[(rb + 4 * p + b) * 8 + cq] = o;
                  }
                }
              }
        ...
        // AM = 1: group gg's 16 x 128 slice of A, 4 uint4 per lane (idx = lane + 32 i):
        // KH = 128: row idx / 8, 16-byte chunk idx % 8, a[i]; KH = 64: K half q,
        // row idx / 4, chunk idx % 4, a[2 q + i]. Stored at ares index idx (row stride KH).
        auto geta = [&](int gg, thread uint4 (&a)[4], int q0, int q1) {
          if constexpr (AM == 1) {
            if constexpr (KH == 128) {
              #pragma clang loop unroll(full)
              for (int i = 0; i < 4; i++) {
                const int idx = int(lane) + 32 * i;
                a[i] = *(const device uint4*)(x + (size_t)(idx >> 3) * K + (size_t)gg * 128 + 16 * (idx & 7));
              }
            } else {
              #pragma clang loop unroll(full)
              for (int q = 0; q < 2; q++) {
                ...
                for (int i = 0; i < 2; i++) {
                  const int idx = int(lane) + 32 * i;
                  a[2 * q + i] = *(const device uint4*)(x + (size_t)(idx >> 2) * K + (size_t)gg * 128 + 64 * q + 16 * (idx & 3));
                }
              }
            }
          }
        };
        auto puta = [&](thread const uint4 (&a)[4], int q0, int q1) {
          if constexpr (AM == 1) {
            if constexpr (KH == 128) {
              #pragma clang loop unroll(full)
              for (int i = 0; i < 4; i++) { ares[sg][int(lane) + 32 * i] = a[i]; }
            } else {
              #pragma clang loop unroll(full)
              for (int q = 0; q < 2; q++) {
                if (q < q0 || q >= q1) { continue; }
                #pragma clang loop unroll(full)
                for (int i = 0; i < 2; i++) { ares[sg][int(lane) + 32 * i] = a[2 * q + i]; }
              }
            }
          }
        };
        ...
        // Rings as in `sourceNarrowInt8Pipelined`; the A ring (AM = 1) mirrors the
        // word ring slot for slot and K half for K half.
        uint32_t wr[PD][NH][8];
        uint4 ar[PD][4];
        half4 shr[CD][NH][2], bhr[CD][NH][2];
        float4 sfr[CD][NH][2];
        ...
        auto mm = [&](int g, int ko) {
          if constexpr (AM == 1) {
            ...
            if (ko == 0) {
              ...
              if (g + 1 < g1) { cA1.load(A.template slice<KH, 16>((g + 1) * 128 + 64, 0)); }
            }
          }
          #endif
          else {
        ...
        auto body = [&](int g, int j) {
          ...
          if constexpr (KH == 64) {
            const int wp = (j + PD - 1) % PD;  // slot holding group g's upper half
            #pragma clang loop unroll(full)
            for (int i = 0; i < CAP; i++) { cT0[i] = 0; cT1[i] = 0; }
            mm(g, 0);
            simdgroup_barrier(mem_flags::mem_threadgroup);
            putw(wr[wp], 1, 2);
            puta(ar[wp], 1, 2);
            if (g + PD < g1) { getw(g + PD, wr[wp], 1, 2); geta(g + PD, ar[wp], 1, 2); }
            simdgroup_barrier(mem_flags::mem_threadgroup);
            mm(g, 64);
          } else {
            mm(g, 0);
          ...
          simdgroup_barrier(mem_flags::mem_threadgroup);
          if (g + 1 < g1) {
            const int wn = j % PD;             // slot holding group g + 1 (its lower half at KH = 64)
            putw(wr[wn], 0, NQ);
            puta(ar[wn], 0, NQ);
            if (g + 1 + PD < g1) { getw(g + 1 + PD, wr[wn], 0, NQ); geta(g + 1 + PD, ar[wn], 0, NQ); }
            simdgroup_barrier(mem_flags::mem_threadgroup);
          }
        };
        {
          ...
          #pragma clang loop unroll(full)
          for (int i = 1; i < PD; i++) {
            if (g0 + i < g1) { getw(g0 + i, wr[i - 1], 0, 2); geta(g0 + i, ar[i - 1], 0, 2); }
          }
          if constexpr (KH == 64) { getw(g0, wr[PD - 1], 1, 2); geta(g0, ar[PD - 1], 1, 2); }
          if (g0 + PD < g1) { getw(g0 + PD, wr[PD - 1], 0, NQ); geta(g0 + PD, ar[PD - 1], 0, NQ); }
          #pragma clang loop unroll(full)
          for (int i = 0; i < CD - 1; i++) {
            if (g0 + i < g1) { getc(g0 + i, shr[i], bhr[i], sfr[i], cr[i]); }
          }
          ...
          if constexpr (AM == 2) {
            ...
            if constexpr (KH == 64) { cA1.load(A.template slice<KH, 16>(g0 * 128 + 64, 0)); }
          }
          #endif
        }
        simdgroup_barrier(mem_flags::mem_threadgroup);
        ...
        threadgroup_barrier(mem_flags::mem_threadgroup);
        if (sg == 0) {
          // As in `sourceNarrowInt8Pipelined`: i = 0, 4, 8, 12, four
          // consecutive columns per step, each summed acc, red[0], red[1],
          // red[2] in that order.
          #pragma clang loop unroll(full)
          for (int h = 0; h < NH; h++) {
            #pragma clang loop unroll(full)
    ...
    private static let kernelNarrowInt8AR = MLXFast.metalKernel(
        name: "bonsai_tensor_packed_matmul_m16_i8ar",
        inputNames: ["x", "w", "scalesT", "biasesT", "ascale", "rowsum", "ksz"],
        outputNames: ["out"],
        source: sourceNarrowInt8AR,
        header: header,
        ensureRowContiguous: true)
    ...
    /// counters) with K = 128 / 64 per slot. The A-resident bodies (L1,
    /// `sourceNarrowInt8AR`, `aresEnabled`): `a64` / `a64pd2` stage each
    /// step's slice of A in threadgroup memory beside B (K = 64 per op, 12 KB
    /// per threadgroup); `b64` / `b128` stage B `[K][N]` and read it
    /// untransposed; `ab64` both; `d128` / `d64` / `d64tn64` hold A in a
    /// left-input cooperative tensor loaded one group ahead (`d64tn64`: one
    /// load for both 32-column halves); `db128` that with `[K][N]` B. Every
    /// one of them within 16 KB of threadgroup memory (two threadgroups per
    /// core). All bitwise identical.
    enum NarrowVariant: Int, CaseIterable {
        ...
        var pd: Int {
            ...
            case .a64, .b64, .b128, .ab64, .d128, .d64, .d64tn64, .db128: return 1
            case .a64pd2: return 2
            }
        }
        var tn: Int { self == .tn64 || self == .d64tn64 ? 64 : 32 }
        var kh: Int {
            [.k64pd1, .k64pd2, .k64pd3, .k64pd4, .k64pc2, .k64pc4,
             .a64, .a64pd2, .b64, .ab64, .d64, .d64tn64].contains(self) ? 64 : 128
        }
        /// The producer / consumer body (K5).
        var producerConsumer: Bool { [.pc2, .k64pc2, .k64pc4].contains(self) }
        /// The A-resident body (L1, `sourceNarrowInt8AR`).
        var aResident: Bool { rawValue >= NarrowVariant.a64.rawValue }
        /// `sourceNarrowInt8AR`'s AM (A: 0 device, 1 threadgroup-staged,
        /// 2 cooperative tensor) and BL (B: 0 `[N][K]`, 1 `[K][N]`).
        var am: Int {
            switch self {
            case .a64, .a64pd2, .ab64: return 1
            case .d128, .d64, .d64tn64, .db128: return 2
            default: return 0
            }
        }
        var bl: Int { [.b64, .b128, .ab64, .db128].contains(self) ? 1 : 0 }
    ...
    static func launchNarrowInt8(
        ...
        case let v where v.producerConsumer:
            return kernelNarrowInt8PC(
                ...
                grid: (n / 32 * 128, 1, 1), threadGroup: (128, 1, 1),
                outputShapes: [[m, n]], outputDTypes: [outputDType])[0]
        case let v where v.aResident:
            return kernelNarrowInt8AR(
                inputs,
                template: template + [
                    ("PD", v.pd), ("TN", v.tn), ("KH", v.kh), ("AM", v.am), ("BL", v.bl),
                ],
    ...
    /// `tn64`, `k64pd1` .. `k64pd4`, `pc2`, `k64pc2`, `k64pc4`, and the
    /// A-resident `a64`, `a64pd2`, `b64`, `b128`, `ab64`, `d128`, `d64`,
    /// `d64tn64`, `db128`) limits the pipelined candidates to those.
    /// The A-resident bodies (L1, `aresEnabled`) come after all of these, in
    /// the first epilogue form, each in its own MLX error scope (one that
    /// fails to compile is dropped alone), exact at FP16 and FP32, within
    /// their own 6 s; they never enter
    /// the record's pick, only the trial: over the record's shapes, the
    /// fastest with A threadgroup-staged (AM = 1), the fastest with A
    /// cooperative (AM = 2) and the fastest with B `[K][N]` (BL = 1), which
    /// together include the fastest overall, and the per-shape set (as the
    /// K5 set).
    /// `DARKBLOOM_BONSAI_TENSOR_ROUTE_NARROW_FORCE=<variant>[/neg|/f32|/base]`
    /// (validation) installs that kernel for every shape once it passes the
    /// FP16 and FP32 self-test, and runs no trial.
    private static func chooseNarrowKernels() -> (NarrowChoice, [NarrowChoice]) {
        ...
        // The A-resident bodies (L1), in self-test order: one of each kind
        // first (a, d, b, a + b), then the rest.
        let aresDefault: [NarrowVariant] = [
            .a64, .d128, .b128, .ab64, .d64, .a64pd2, .b64, .d64tn64, .db128,
        ]
        var variants = defaultVariants
        var aresVariants = aresDefault
        switch knob("DARKBLOOM_BONSAI_TENSOR_ROUTE_NARROW_PIPELINE") {
        case "off", "0", "false", "no", "v0":
            variants = []
            aresVariants = []
        case .some(let value):
            ...
            if !named.isEmpty, named.allSatisfy({ $0 != nil }) {
                ...
                aresVariants = variants.filter(\.aResident)
            }
        case nil: break
        }
        variants = variants.filter { !$0.aResident }
        if !aresEnabled { aresVariants = [] }
        let candidates = forms.map { NarrowKernel(variant: .v0, form: $0) }
            + variants.flatMap { v in forms.map { NarrowKernel(variant: v, form: $0) } }
        // In the first epilogue form only (the negated offset with FP16 scales
        // unless `..._NARROW_EPILOGUE` says otherwise): the A axis is what
        // they vary, and one form keeps their self-test within its budget.
        let aresCandidates = aresVariants.map { NarrowKernel(variant: $0, form: forms[0]) }
        // Validation: the one kernel to install for every shape, no trial.
        var forcedKernel: NarrowKernel?
        if let raw = knob("DARKBLOOM_BONSAI_TENSOR_ROUTE_NARROW_FORCE"), !raw.isEmpty {
            let parts = raw.split(separator: "/").map(String.init)
            if let v = NarrowVariant(name: parts[0]) {
                let form: NarrowEpilogue
                switch parts.count > 1 ? parts[1] : "" {
                case "base": form = .base
                case "f32": form = .negativeBiasF32Scales
                case "neg": form = .negativeBias
                default: form = forms[0]
                }
                forcedKernel = NarrowKernel(variant: v, form: form)
            }
        }
        ...
        do {
            try withError { error in
                ...
                // L1: each A-resident kernel in its own error scope, so a body
                // this toolchain cannot compile (or that errs) is dropped alone
                // and nothing else changes; FP16 and FP32 outputs bit for bit.
                let aresStart = DispatchTime.now().uptimeNanoseconds
                var aresDropped: [String] = []
                for kernel in aresCandidates {
                    if Double(DispatchTime.now().uptimeNanoseconds - aresStart) / 1e6 > 6000 {
                        skipped.append(kernel)
                        continue
                    }
                    do {
                        let exact = try withError { scope -> Bool in
                        ...
                        if exact { passed.append(kernel) } else { aresDropped.append("\(kernel) (mismatch)") }
                    } catch {
                        aresDropped.append("\(kernel) (error)")
                    }
                }
                log += " self-test passed [" + passed.map(\.description).joined(separator: " ") + "]"
                if !aresDropped.isEmpty {
                    log += " dropped [" + aresDropped.joined(separator: " ") + "]"
                }
                if !skipped.isEmpty {
                ...
                if let forced = forcedKernel {
                    var usable = forced == .original
                    if !usable, passed.contains(forced) { usable = try exactF32(forced) }
                    if usable {
                        fallback = forced
                        byShape = [:]
                        log += " forced \(forced), no trial;"
                        return
                    }
                    log += " forced \(forced) unavailable;"
                }
                ...
                // L1: in total over the record's shapes, the fastest A-resident
                // kernel of each A mode (threadgroup-staged, cooperative) and
                // the fastest with B `[K][N]`, so the synthetic timing (which
                // mispicks) cannot cut a whole mode from the rounds; every
                // A-resident body is in one of the three, so the fastest
                // overall is too. Then the A-resident per-shape set (as the K5
                // set). Each exact at FP32; a class's pick already taken for
                // another class is not repeated.
                let aresPool = passed.filter { $0.variant.aResident }
                    .sorted { recordTotal($0) < recordTotal($1) }
                var aresPicked: [NarrowKernel] = []
                let aresClasses: [(NarrowVariant) -> Bool] = [{ $0.am == 1 }, { $0.am == 2 }, { $0.bl == 1 }]
                for inClass in aresClasses {
                    for kernel in aresPool where inClass(kernel.variant) {
                        if aresPicked.contains(kernel) { break }
                        if try exactF32(kernel) {
                            aresPicked.append(kernel)
                            break
                        }
                    }
                }
                shortlist += aresPicked.map { ($0, [:]) }
                while true {
                    let pool = aresPool.filter { !failedF32.contains($0) }
                    guard !pool.isEmpty else { break }
                    func fastest(_ cost: (NarrowKernel) -> Double) -> NarrowKernel {
                        pool.min { cost($0) < cost($1) }!
                    }
                    let aresTotal = fastest { k in
                        zip(timings[k]!, reference).reduce(0) { $0 + $1.0 / $1.1 }
                    }
                    var aresByShape: [[Int]: NarrowKernel] = [:]
                    for (index, shape) in tuned.enumerated() {
                        aresByShape[[shape.0, shape.1]] = fastest { timings[$0]![index] }
                    }
                    var clean = true
                    for kernel in Set([aresTotal] + Array(aresByShape.values)) {
                        if try !exactF32(kernel) { clean = false }
                    }
                    if clean {
                        shortlist.append((aresTotal, aresByShape))
                        break
                    }
                }
    ...
    static func installIfNeeded() {
        ...
        if verifyEnabled, verifyForm == .staged8, signedCodes {
            if Qwen35VerifyProducerQ8.enabled {
        ...
        HadamardQuantizedLinear.tensorPackedMatmul = {
            ...
            case .native2b:
                packedKernel = kernel
                template.append(("VEC", nativeVectorStores ? 1 : 0))
            case .staged8:
...
enum Qwen35HeadTopTwo {
    static let enabled: Bool = {
        // Off by default here (`MLXFAST_HEAD_TOP2=1` enables it).
...
extension Qwen35TensorPackedMatmul {
    ...
    private static let kernelNarrowInt8ARTop2 = headTop2Kernel(
        "bonsai_tensor_packed_matmul_m16_i8ar_top2", sourceNarrowInt8AR, columns: "TN")
    ...
    static func launchNarrowInt8Top2(
        ...
        case let v where v.producerConsumer:
            ...
            partial = launch(
                ...
                outputShapes: shapes, outputDTypes: dtypes)
        case let v where v.aResident:
            guard let launch = kernelNarrowInt8ARTop2 else { return nil }
            partial = launch(
                inputs,
                template: template + [
                    ("PD", v.pd), ("TN", v.tn), ("KH", v.kh), ("AM", v.am), ("BL", v.bl),
