...
public final class Qwen35DFlash2Assistant: CBv2MTPBlockDrafter, @unchecked Sendable {
    ...
    func warmSpeculativeShapes(serving: (any LanguageModel)? = nil) {
        ...
        warmTargetPrefill()
        warmDrafter()
        if let serving { warmEngineRound(serving: serving) }
        Stream().synchronize()
        Memory.clearCache()
    }
    ...
    private func warmEngineRound(serving: any LanguageModel) {
        ...
        // The engine and its arrays are released as the task above unwinds;
        // let those frees land before `warmSpeculativeShapes` drains, so the
        // cache the served phases see is the one a cold load leaves.
        for _ in 0 ..< 50 {
            Stream().synchronize()
            Memory.clearCache()
            if Memory.cacheMemory == 0 { break }
            usleep(2_000)
        }
    }
