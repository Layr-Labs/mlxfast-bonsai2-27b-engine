...
// Ported from upstream 2118561: the Qwen3VL image path must apply the sRGB
// tone curve before resampling/normalizing, like the sibling Qwen25VL/Qwen2VL
// image paths and this processor's own video path already do.
...
final class Qwen3VLToneCurveTests: XCTestCase {
    ...
    /// background and a faint headline. 64 is a multiple of the merge factor
    /// (patch 16 × merge 2 = 32), so the target size equals the input size
    /// and resampling does not perturb the values.
