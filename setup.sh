#!/usr/bin/env bash
# Bootstrap system tools and build the Swift-only Ternary Bonsai 2 27B harness.
set -euo pipefail

# REPOINTED onto the Ternary Bonsai 2 27B MLX 2-bit pack, in
...
# Tests/MLXFastTests/Model/, and fixtures/bonsai2_27b_mlx_v1_track.json's
# target.upstream_model_id / target.upstream_revision -- four independent
# sources that must agree byte-for-byte on this pin.
#
# This pack is NOT our own conversion: it is fetched directly from the
# published MLX 2-bit release. It carries NO MTP head; the track's head is a
# separate published export that ./setup-mtp-head.sh stages beside it. The
# repository is PUBLIC, so the
...
REFERENCE_MODEL_REPO="${MLXFAST_REFERENCE_MODEL_REPO:-prism-ml/Ternary-Bonsai-2-27B-mlx-2bit}"
REFERENCE_REVISION="${MLXFAST_REFERENCE_REVISION:-3f926b415992eaa2ae9dd7b573706494d6bbf787}"
...
DEFAULT_REFERENCE_BASE_URL="https://huggingface.co/${REFERENCE_MODEL_REPO}/resolve/${REFERENCE_REVISION}"
...
REFERENCE_MANIFEST_PATH="${MLXFAST_REFERENCE_MANIFEST_PATH:-fixtures/reference_bonsai2_27b_2bit.sha256}"
...
# 22 GiB covers the ~8.1 GiB pinned snapshot (8608721032 bytes across 19
# manifest records), the ~0.23 GiB MTP head beside it, and headroom for the
# transform's output alongside both, at the same ~2.5x snapshot-to-floor ratio
# the retired Gemma default used (40 GiB floor for a 14.6 GiB snapshot).
REFERENCE_MIN_FREE_GIB="${MLXFAST_REFERENCE_MIN_FREE_GIB:-22}"
# The pinned checkpoint has 4 safetensors shards, so one job per shard keeps
# the link saturated. Override with MLXFAST_REFERENCE_DOWNLOAD_JOBS.
...
DEFAULT_REFERENCE_DIR="reference_weights/Ternary-Bonsai-2-27B-mlx-2bit"
...
builds mlx.metallib, and downloads the Nemotron 3.5 30B A3B MLX 4-bit reference
...
# (fixtures/reference_bonsai2_27b_2bit.sha256) pins README.md, NOTICE.txt,
# PACK-RUNTIME.md, chat_template.jinja, config.json, files.json,
# generation_config.json, hadamard.json, preprocessor_config.json,
# reload-validation.json, the five runtime/*.py files, tokenizer.json,
# tokenizer_config.json and tokenizer-validation.json, plus the ONE safetensors
# shard -- 19 records, NO .gitattributes, NO LICENSE, NO image files. THE PACK
# PUBLISHES NO model.safetensors.index.json: it is one shard, and the index is
# optional exactly there. hadamard.json is NOT optional -- the packed loader
# refuses without it.
#
# The MTP head (fixtures/reference_bonsai2_27b_mtp_head_4bit.sha256) is the
# OTHER artifact. It pins README.md, config.json,
# model.safetensors.index.json and its one shard -- 4 records, no tokenizer of
# any kind, because the head borrows the target's.
#
...
# THE HEAD IS THE SECOND ARTIFACT. The pinned pack declares
# `mtp_num_hidden_layers: 0` and ships no `mtp.*` tensor, so the MTP head is a
# separate published export with its own pin file. ./setup-mtp-head.sh points
# THIS downloader at it, which is why the head is fetched and verified by the
# same resumable, hash-verified path the target is.
#
# The recursion guard is the same variable ./setup-mtp-head.sh exports, so the
# delegated run provisions the head and stops there.
provision_mtp_head() {
  if [[ "${MLXFAST_SKIP_MTP_HEAD:-0}" == "1" ]]; then
    echo "${SETUP_LOG_LABEL}: MLXFAST_SKIP_MTP_HEAD=1; not provisioning the MTP head"
    return 0
  fi
  if [[ ! -x "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)/setup-mtp-head.sh" ]]; then
    echo "${SETUP_LOG_LABEL}: setup-mtp-head.sh is missing or not executable; the MTP head is not staged" >&2
    return 1
  fi
  echo "${SETUP_LOG_LABEL}: provisioning the MTP head (./setup-mtp-head.sh)"
  "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)/setup-mtp-head.sh"
}

# THE DFLASH 2 DRAFTER IS THE THIRD ARTIFACT, and it is there for the same
# reason the head is: the pack carries no drafter, so this track's second
# declarable decoder is its own published export with its own pin file.
# ./setup-dflash-drafter.sh points THIS downloader at it.
#
# The recursion guard is the same variable ./setup-dflash-drafter.sh exports, so
# the delegated run provisions the drafter and stops there.
provision_dflash_drafter() {
  if [[ "${MLXFAST_SKIP_DFLASH_DRAFTER:-0}" == "1" ]]; then
    echo "${SETUP_LOG_LABEL}: MLXFAST_SKIP_DFLASH_DRAFTER=1; not provisioning the DFlash 2 drafter"
    return 0
  fi
  if [[ ! -x "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)/setup-dflash-drafter.sh" ]]; then
    echo "${SETUP_LOG_LABEL}: setup-dflash-drafter.sh is missing or not executable; the DFlash 2 drafter is not staged" >&2
    return 1
  fi
  echo "${SETUP_LOG_LABEL}: provisioning the DFlash 2 drafter (./setup-dflash-drafter.sh)"
  "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)/setup-dflash-drafter.sh"
}
