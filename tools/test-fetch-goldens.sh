...
# ============================================================================
# --public: the public captures for local runs
# ============================================================================
# The captures are R2 objects, never repository files. --public reads their pins
# from the contract's public_captures and writes each one to its r2_path under
# the repository root, which is where a stand-in root puts them here.

# --- case 15: the SHIPPED fixture is armed, and --public needs the endpoint ---
# The coupling case. It runs the real script against the real fixture with no
# endpoint at all. The captures are pinned, so the missing endpoint must be the
# reason given, and nothing may be written.
before="$(cd "${SCRIPT_DIR}" && git status --porcelain --untracked-files=all | wc -l | tr -d ' ')"
if env -u R2_BUCKET_ENDPOINT "${FETCH}" --public >"${WORK}/case15.log" 2>&1; then
  fail "case 15: --public succeeded against the shipped fixture with no endpoint"
elif ! grep -q "R2_BUCKET_ENDPOINT is not set" "${WORK}/case15.log"; then
  fail "case 15: refused for the wrong reason ($(tail -3 "${WORK}/case15.log"))"
elif grep -q "public captures for this track are not recorded yet" "${WORK}/case15.log"; then
  fail "case 15: the shipped captures still read as pending"
elif [ "$(cd "${SCRIPT_DIR}" && git status --porcelain --untracked-files=all | wc -l | tr -d ' ')" != "${before}" ]; then
  fail "case 15: a refused --public wrote into the checkout"
else
  pass "--public reads the shipped pins and refuses by name when the endpoint is missing"
fi

PUB_KEY_I="correctness_prompts/bonsai2-27b-mlx-v1/public-local-iterate.golden.json"
PUB_KEY_S="correctness_prompts/bonsai2-27b-mlx-v1/public-local-submit.golden.json"
printf '{"version":1,"case":"public-local-iterate"}\n' > "${WORK}/bucket/${PUB_KEY_I}"
printf '{"version":1,"case":"public-local-submit"}\n' > "${WORK}/bucket/${PUB_KEY_S}"
I_SHA="$(pin_sha "${WORK}/bucket/${PUB_KEY_I}")"; I_BYTES="$(pin_bytes "${WORK}/bucket/${PUB_KEY_I}")"
S_SHA="$(pin_sha "${WORK}/bucket/${PUB_KEY_S}")"; S_BYTES="$(pin_bytes "${WORK}/bucket/${PUB_KEY_S}")"

# public_root DIR S_BYTES -- a stand-in repo root with the loopback copy and a
# contract that pins both captures; the second byte pin is a parameter so the
# truncation case can make exactly one thing wrong.
public_root() {
  mkdir -p "$1/tools" "$1/fixtures"
  cp "${RELAXED}" "$1/tools/fetch-goldens-loopback.sh"
  chmod +x "$1/tools/fetch-goldens-loopback.sh"
  cat > "$1/fixtures/bonsai2_27b_mlx_v1_track.json" <<PUBJSON
{
  "timed_prompt_pool": [],
  "hidden_correctness_golden": {"sha256": "BONSAI2-27B-MLX-V1-PENDING-ORGANIZER", "bytes": 0},
  "public_captures": {
    "local_iterate": {"r2_path": "${PUB_KEY_I}", "sha256": "${I_SHA}", "bytes": ${I_BYTES}},
    "local_submit": {"r2_path": "${PUB_KEY_S}", "sha256": "${S_SHA}", "bytes": $2}
  }
}
PUBJSON
}

# --- case 16: --public fetches both captures, anonymously, to their r2_path --
public_root "${WORK}/pubrepo" "${S_BYTES}"
if env -u R2_ACCESS_KEY_ID -u R2_SECRET_ACCESS_KEY R2_BUCKET_ENDPOINT="${ENDPOINT}" \
   "${WORK}/pubrepo/tools/fetch-goldens-loopback.sh" --public >"${WORK}/case16.log" 2>&1; then
  ok16=1
  [[ "$(pin_sha "${WORK}/pubrepo/${PUB_KEY_I}")" == "${I_SHA}" ]] \
    || { fail "case 16: the local_iterate capture is missing or does not match its pin"; ok16=0; }
  [[ "$(pin_sha "${WORK}/pubrepo/${PUB_KEY_S}")" == "${S_SHA}" ]] \
    || { fail "case 16: the local_submit capture is missing or does not match its pin"; ok16=0; }
  compgen -G "${WORK}/pubrepo/correctness_prompts/bonsai2-27b-mlx-v1/*.partial" >/dev/null \
    && { fail "case 16: a .partial file survived a successful run"; ok16=0; }
  perms="$(ls -l "${WORK}/pubrepo/${PUB_KEY_I}" | cut -c2-10)"
  [[ "${perms}" == "r--r--r--" ]] \
    || { fail "case 16: the capture is ${perms}, expected r--r--r-- (0444)"; ok16=0; }
  grep -q "fetched 2 capture(s), kept 0" "${WORK}/case16.log" \
    || { fail "case 16: the run did not report 2 fetched captures"; ok16=0; }
  [[ "${ok16}" == "1" ]] && pass "--public fetched both captures without credentials and verified them"
else
  fail "case 16: --public refused a healthy pinned pair ($(tail -3 "${WORK}/case16.log"))"
fi

# --- case 17: a second --public run fetches nothing -----------------------------
if env -u R2_ACCESS_KEY_ID -u R2_SECRET_ACCESS_KEY R2_BUCKET_ENDPOINT="${ENDPOINT}" \
   "${WORK}/pubrepo/tools/fetch-goldens-loopback.sh" --public >"${WORK}/case17.log" 2>&1 \
   && grep -q "fetched 0 capture(s), kept 2" "${WORK}/case17.log"; then
  pass "a second --public run kept both pinned captures and fetched nothing"
else
  fail "case 17: the second --public run did not keep both captures ($(tail -3 "${WORK}/case17.log"))"
fi

# --- case 18: a capture that does not match its pin is refused, nothing left --
public_root "${WORK}/pubrepo-short" "$((S_BYTES + 1))"
if env -u R2_ACCESS_KEY_ID -u R2_SECRET_ACCESS_KEY R2_BUCKET_ENDPOINT="${ENDPOINT}" \
   "${WORK}/pubrepo-short/tools/fetch-goldens-loopback.sh" --public >"${WORK}/case18.log" 2>&1; then
  fail "case 18: --public accepted a capture that is a byte short of its pin"
elif [[ -e "${WORK}/pubrepo-short/${PUB_KEY_S}" ]] \
  || compgen -G "${WORK}/pubrepo-short/correctness_prompts/bonsai2-27b-mlx-v1/*.partial" >/dev/null; then
  fail "case 18: refused but left the capture or a .partial behind"
elif grep -q "byte-count mismatch for ${PUB_KEY_S}" "${WORK}/case18.log"; then
  pass "--public refuses a capture that does not match its pin and leaves no bytes"
else
  fail "case 18: refused for the wrong reason ($(tail -3 "${WORK}/case18.log"))"
fi

# --- case 19: --public takes no other argument -------------------------------
if R2_BUCKET_ENDPOINT="${ENDPOINT}" "${WORK}/pubrepo/tools/fetch-goldens-loopback.sh" \
   --public --out "${WORK}/elsewhere" >"${WORK}/case19.log" 2>&1; then
  fail "case 19: --public accepted --out"
elif grep -q -- "--public takes no other argument" "${WORK}/case19.log"; then
  pass "--public refuses any other argument"
else
  fail "case 19: refused for the wrong reason ($(tail -3 "${WORK}/case19.log"))"
fi
