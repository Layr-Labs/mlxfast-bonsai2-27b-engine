...
What it asserts, in order:
  ...
  6. scoring constants  match the PINNED constants for this manifest's OWN trackId
                        ...
                        docs/bonsai2-27b-port-notes.md (benchmark.json
...
FAIL. THIS REPOSITORY HAS NO SUBMODULES: benchd stopped being a source
submodule when it became a pinned prebuilt, the engine fork is a vendored tree
rather than a gitlink, and .gitmodules is gone, so gitlink_paths() is empty and
the downgrade applies to nothing. Every command target lives in this repository
(./setup.sh, ./tools/fetch-benchd.sh,
./tools/bonsai2-27b-measure-and-score.sh) and all three are fully verified
in CI. The flag is kept for the next submodule, not removed as dead.
"""
...
#
# QWEN3.8-27B-MTP-V1 sources. That track's own repository, not this tree:
#   floor / ceiling / aggregation / median rule / pairs per prompt
#       its own contract fixture -> scoring_semantics
...
#       docs/nemotron35-30b-a3b-port-notes.md section 4 for the full split).
#   pairsPerCohort = 4
...
#            qwen3.8-125b-a6b-mlx-v1 release branch;
#         3. David 2026-08-26 -- RULED 4 (this value), returning to the
#            brief's sample count on sample-mass grounds.
#       CHANNEL AGREEMENT IS A TWO-PHASE LANDING HERE, and that is deliberate:
#       the benchd side (PAIRS_PER_COHORT_TARGET 2 -> 4) must MERGE AND
#       PUBLISH before a resolve can obtain a benchd that compiles 4. Until
#       that publish lands, the channel tip still compiles
...
#       caveat-based warn path at all. See docs/nemotron35-30b-a3b-port-notes.md
#       section 4 (which this dict's value must always match) for the full citation
...
#       only ever run locally; docs/nemotron35-30b-a3b-port-notes.md remains
...
#       tools/bonsai2-27b-measure-and-score.sh, so it does not compare the manifest
...
# nemotron3.5-30b-a3b-mlx-v1: the manifest block states the values the contract
# fixture enforces (official_pairs 3, decode_speedup_floor 0.95 -- David
# 2026-09-17, "3 pairs"; the ranked run scores the lower-median pair, never a
# mean). benchd reads the fixture, not this block; the block is the manifest's
# statement of the same numbers and this registry keeps the two in step.
_NEMOTRON35_30B_A3B_MLX_V1_SCORING = {
    "mode": "nemotron-native-mtp-paired-composite",
    "scoredBatchSize": 1,
    "kvBackend": "contiguous",
...
# bonsai2-27b-mlx-v1: THIS TRACK. The port carried the source track's scoring
# block over unchanged, because the ruling behind it -- the composite
# prefill^0.25 * decode^0.75 over a lower-median of 3 pairs -- is the family's
# and not the model's. The only value that moved is `mode`, which names the
# track. benchd reads the contract fixture, not this block; the block is the
# manifest's statement of the same numbers and this registry keeps the two in
# step.
_BONSAI2_27B_MLX_V1_SCORING = {
    "mode": "bonsai2-native-mtp-paired-composite",
    "scoredBatchSize": 1,
    "kvBackend": "contiguous",
    "decodeSpeedupFloor": 0.95,
    "decodeSpeedupCeiling": 5.0,
...
EXPECTED_SCORING_BY_TRACK = {
    ...
    "nemotron3.5-30b-a3b-mlx-v1": _NEMOTRON35_30B_A3B_MLX_V1_SCORING,
    "bonsai2-27b-mlx-v1": _BONSAI2_27B_MLX_V1_SCORING,
}
...
# docs/nemotron35-30b-a3b-port-notes.md section 4). This needle check is scoped entirely
...
#
# `Vendor/mlx-swift-lm` is NOT on this list any more. The engine fork used to be
# a gitlink, and a gitlink names a commit rather than bytes in this tree, so an
# editable entry over it would have let a submission move the engine to a commit
# nothing here verified. The fork is now a VENDORED TREE at a pinned commit, the
# way `Vendor/mlx-swift` already was, so its files are ordinary tracked bytes
# that the surface gate diffs like any other source. The ten Nemotron 3.5
# Lightning model files under `Vendor/mlx-swift-lm/Libraries/MLXLLM/Models/` are
# declared editable by name; everything else in the fork stays trusted because
# nothing names it.
FORBIDDEN_EDITABLE = (
    "benchd",
    ".gitmodules",
...
# by contract. The engine itself is the vendored Vendor/mlx-swift-lm tree, and
# only the ten Nemotron model files in it are declared editable, so there is no
# sandboxed participant-facing source directory left for this scope to argue
# about.
#
