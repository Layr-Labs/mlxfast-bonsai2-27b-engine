...
prev=""
for arg in "$@"; do
  if [[ "${prev}" == "--mtp-depth" ]]; then MTP_DEPTH="${arg}"; prev=""; continue; fi
  if [[ "${prev}" == "--dflash-depth" ]]; then DFLASH_DEPTH="${arg}"; prev=""; continue; fi
  if [[ "${arg}" == "--mtp-depth" ]]; then prev="--mtp-depth"; continue; fi
  if [[ "${arg}" == "--dflash-depth" ]]; then prev="--dflash-depth"; continue; fi
  if [[ "${arg}" == --mtp-depth=* ]]; then MTP_DEPTH="${arg#--mtp-depth=}"; continue; fi
  if [[ "${arg}" == --dflash-depth=* ]]; then DFLASH_DEPTH="${arg#--dflash-depth=}"; continue; fi
  prev=""
done
for arg in "$@"; do
  case "${arg}" in
...
# DIVERGENCE FROM UPSTREAM (edit 1 of 2): the TRACK ID. The variable name is the
# HARNESS PROTOCOL'S, not this track's: benchd reads this exact spelling on every
# track it scores, so it stays as it is. `benchd iterate` REQUIRES
...
# manifest's `trackId` (the PLATFORM id, bonsai2-27b-mlx-v1 -- NOT the bench
# channel branch, which names the channel; see tools/fetch-benchd.sh).
...
# THE CONTRACT, ON EVERY MODE. benchd reads the measurement window, the model
# identity and the scoring values from the track fixture and refuses a run it
# was not given one for (WINDOW-SHAPE-UNDECLARED-FOR-TRACK), local modes
# included. The fixture is fixtures/bonsai2_27b_mlx_v1_track.json.
CONTRACT_FIXTURE="${FACADE_DIR}/../fixtures/bonsai2_27b_mlx_v1_track.json"
if [[ ! -f "${CONTRACT_FIXTURE}" ]]; then
  echo "benchmark.sh: every run requires the track contract fixture at ${CONTRACT_FIXTURE}" >&2
  exit 1
fi
benchd_args+=(--contract "${CONTRACT_FIXTURE}")
# THE SPECULATIVE LEG. `--mtp-depth <N>` asks benchd for the native-MTP leg at
# draft depth N on the timed decode window, and `--dflash-depth <N>` asks for
# the DFlash 2 block leg at the same depth; absent, the window is serial.
#
# The two flags are mutually exclusive, because they name ONE leg's decoder.
# benchd itself takes `--mtp-depth` or `--candidate-spec`, never both, so the
# DFlash 2 depth travels as the explicit candidate spec.
if [[ -n "${MTP_DEPTH}" && -n "${DFLASH_DEPTH}" ]]; then
  echo "benchmark.sh: --mtp-depth and --dflash-depth name the same leg's decoder; pass exactly one" >&2
  exit 1
fi
if [[ -n "${MTP_DEPTH}" ]]; then
  [[ "${MTP_DEPTH}" =~ ^[1-9][0-9]*$ ]] || { echo "benchmark.sh: --mtp-depth must be a positive integer (got '${MTP_DEPTH}')" >&2; exit 1; }
  benchd_args+=(--mtp-depth "${MTP_DEPTH}")
  # The head is a separate export. benchd spawns the worker with --weights
  # alone, so the worker reads the head directory from its environment.
  export BENCH_WORKER_DRAFTER="${BENCH_WORKER_DRAFTER:-${FACADE_DIR}/../reference_weights/Qwen3.8-27B-MTP-4bit}"
fi
if [[ -n "${DFLASH_DEPTH}" ]]; then
  [[ "${DFLASH_DEPTH}" =~ ^[1-9][0-9]*$ ]] || { echo "benchmark.sh: --dflash-depth must be a positive integer (got '${DFLASH_DEPTH}')" >&2; exit 1; }
  (( DFLASH_DEPTH <= 16 )) || { echo "benchmark.sh: --dflash-depth must be at most 16 (got '${DFLASH_DEPTH}'); the DFlash 2 block is depth plus one" >&2; exit 1; }
  benchd_args+=(--candidate-spec "{\"mode\":\"dflash\",\"dflash\":{\"depth\":${DFLASH_DEPTH}}}")
  # The DFlash 2 drafter is its own separate export, so it travels the same way
  # the MTP head does: one drafter directory in the worker's environment.
  export BENCH_WORKER_DRAFTER="${BENCH_WORKER_DRAFTER:-${FACADE_DIR}/../reference_weights/Qwen3.8-27B-DFlash2}"
fi
if [[ "${OFFICIAL}" == "1" ]]; then
  enforce_official_sandbox
  export MLXFAST_USE_RUNTIME_WORKER="${USE_RUNTIME_WORKER}"
  # ARM GATE (David 2026-08-26): --mode official seals only when the fixture
  # declares official_scoring_enabled: true. It is false today, so official
  # runs refuse until the arming change flips it true.
else
