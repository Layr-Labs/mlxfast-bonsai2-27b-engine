...
# Vendor/mlx-swift-lm files are editablePaths entries; the fork was a submodule
# then, and a gitlink is not an editable path. Both subjects left the tree, so
# both sets of assertions left with them.
#
# The fork is a VENDORED TREE again (Nemotron 3.5 Lightning), so its ten model
# files ARE editablePaths entries once more. That did not add assertions: the
# loop those 23 assertions belonged to was deleted, not disabled, and this
# change re-declares the paths without re-adding the loop.
...
# bonsai2-27b-mlx-v1 manifest PR). Note which way that staleness pointed:
...
#
# The model files that decide the head's geometry ARE in this tree: the engine
# fork is a vendored tree, and the ten Nemotron 3.5 Lightning model files in it
# are editablePaths entries. That is a MODEL-SOURCE surface, not a weights
# surface. It grants no way to upload head bytes, which is what the loop below
# actually guards.
...
assert_equal "head-surface/editablePaths holds exactly 97 entries" \
...
for count_site in README.md TASK.md docs/participant-contract.md; do
  assert_equal "head-surface/${count_site} states the same 97 entries" \
    "$(grep -c 'lists 97 entries' "${REPO_ROOT}/${count_site}")" "1"
done
assert_equal "head-surface/the Swift count pin states 97" \
  "$(grep -c 'editablePaths.count == 97' "${REPO_ROOT}/Tests/MLXFastTests/Gemma4BenchmarkManifestTests.swift")" \
  "1"
