...
#      existing one.
#   4. Render com.bonsai2.box-runner.plist.template into a ready-to-copy
...
render_plist() {
  local template="${SCRIPT_DIR}/com.bonsai2.box-runner.plist.template"
  local out="${SCRIPT_DIR}/com.bonsai2.box-runner.plist"
...
install.sh does NOT load the daemon. To go live, an operator runs (as root):
  ...
  sudo cp "${out}" /Library/LaunchDaemons/com.bonsai2.box-runner.plist
