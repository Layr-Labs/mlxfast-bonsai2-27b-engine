#!/usr/bin/env bash
# VENDORED VERBATIM from the production track, per the CUDA-track provisioning
# ruling (issue #36: "hidden material ... following the challenger's exact
# pattern"). Source:
#
#   Layr-Labs/qwen-3.8-mtp-challenge
#   .github/scripts/download-r2-object.sh
#   @ 5d029178765cf727e7ee530b0b4c731d566f908a
#   sha256 23db28a6d76dfe15b53c0877127ef6ccd4bb67b7d4a65406b130dc58789c4770
#
# Only this header is added; not one byte of the body diverges. Every comment
# below records a failure that was actually diagnosed against real R2 on the
# production track, and re-deriving this signer would re-earn those failures.
# If the upstream copy changes, re-vendor it -- do not patch it here.
#
# THIS SCRIPT IS GET-ONLY, AND THAT IS A STANDING CONSTRAINT, not an accident of
# what was needed first. Provisioning UPLOADS (goldens, prompt pools, GPQA
# reference) require the operator's per-instance authorization naming the
# objects; see docs/CUDA-TRACK-PROVISIONING.md.
set -euo pipefail
